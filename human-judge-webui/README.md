...
```bash
python human-judge-webui/app.py serve --host 0.0.0.0 --port 8765
```
...
3. 先提交 OQ（整体质量），服务器立即保存并锁定，再显示 IF、VQ、TC、NC。OQ 结合任务满足、音画整体性、编辑完成度、自然性和可发布性判断。
4. 完成四项细分比较，可选填理由，点击“保存并抽取下一组”。重复点击或网络重试不会产生重复记录。OQ 与四个分项必须属于同一比较。
5. 无法播放或无法评价时填写原因并跳过。跳过与未完成记录单独保留，不计入已完成比较数；已提交的 OQ 也保留供审计。

| 指标 | 判断内容 |
| --- | --- |
| IF | 主体、事件、风格、情绪或叙事意图是否满足提示词 |
| VQ | 清晰度、构图、主体突出程度和技术缺陷 |
| TC | 相邻片段的视觉语义、运动与构图衔接 |
| NC | 符合提示词的结构、推进过程或故事／情绪弧线 |
| OQ | 给定任务、BGM 和目标时长下的独立整体质量判断 |

每项采用同一**五档相对偏好**：`+2=A明显更好`，`+1=A略好`，`0=相当`，`-1=B略好`，`-2=B明显更好`。四个分项的构念对应 [VLM rubric](../docs/vlm_judge_rubric.md)，但这是 pairwise 相对评价，不是给 A/B 各自打 1–5 绝对分。BCS/AEC 不在人评范围；人工判断不进入自动 `Quality`，也不写入 `runs/` 或 `eval_results/`。

## 配置

默认配置位于 [config.json](config.json)：五个 baseline 全部启用，CutMaster 指向 `runs/cutmaster_overlap_beat`。可复制为 `config.local.json`，通过 `--config human-judge-webui/config.local.json` 使用。
...
```bash
python human-judge-webui/app.py prepare-media
```
...
默认数据库：`human-judge-webui/data/judgments.sqlite3`，使用 WAL 支持多人写入。每条记录保存匿名 participant ID、标注者代号、任务、A/B 真实方法映射、创建／OQ／提交时间、五项偏好、备注、状态及媒体版本信息；study 表保存配置与任务快照。服务器重启后可继续。

```bash
python human-judge-webui/app.py export
# 或指定配置和导出目录
python human-judge-webui/app.py export --config human-judge-webui/config.json --output human-judge-webui/exports
```

导出三个带 UTC 时间戳的文件，已有导出不会被覆盖：

- `.csv`：仅完成的比较，每组五行，一行一个指标；包含 `preference` 和按实际方法解析的 `winner`（或 tie）。
...
```bash
python -m unittest discover -s human-judge-webui -p 'test_*.py' -v
```

测试覆盖身份隔离、抽样、并发续评、OQ 锁定、严格评分校验、幂等提交、跳过、配置变更隔离、导出和 HTTP Range。测试使用临时数据，不会写入正式人评数据库。
